 ...
 *
 *   GOODVIBES_DAEMON_RELEASE_NOTES="$(cat <<'EOF'
 *   Added: ...
 *   Fixed: ...
 *   EOF
...
const notesContent = process.env.GOODVIBES_DAEMON_RELEASE_NOTES;
if (notesContent !== undefined && notesContent.trim().length === 0) {
  console.error('Error: GOODVIBES_DAEMON_RELEASE_NOTES is set but empty. Unset it, or give it the release notes.');
  process.exit(1);
}
...
// Removed explicitly below on both the success and failure paths.
const notesDir = mkdtempSync(join(tmpdir(), 'gv-daemon-release-notes-'));

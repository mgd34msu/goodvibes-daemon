...
#
# ONE CURL, FOUR PRODUCTS, FOUR REPOSITORIES
#
# GoodVibes is a daemon, a terminal app, a personal agent and a browser
# operator surface. Each is built and released from its own repository now, and
# this script is what keeps installing all of them a single command:
#
#   goodvibes-daemon   mgd34msu/goodvibes-daemon  binary + the sqlite-vec addon
#   goodvibes          mgd34msu/goodvibes-tui     binary
#   goodvibes-agent    mgd34msu/goodvibes-agent   binary + its browser driver
#   the web UI         mgd34msu/goodvibes-webui   a built bundle the daemon serves
#
# A release tag is resolved per repository, and every file is verified against
# THAT repository's own SHA256SUMS.txt before it is placed. A missing manifest
# entry is a hard failure, never a skip. Binaries land in ~/.local/bin (override
# with GOODVIBES_INSTALL_DIR) and the doctor install self-check runs when
# possible.
#
# The agent's browser driver archive is downloaded, verified and extracted to
# $INSTALL_DIR/playwright-core, which is what makes goodvibes-agent able to
# drive a browser at all (a compiled binary carries no node_modules).
#
# The platform's sqlite-vec native addon is placed at
# $INSTALL_DIR/lib/sqlite-vec-<os>-<arch>/vec0.<suffix> — the path the SDK
# resolves next to the running binary — so semantic vector search works for
# pure-binary installs (npm installs already ship it in the platform package).
# One copy in $INSTALL_DIR/lib serves goodvibes, goodvibes-daemon, and
# goodvibes-agent, since all three share $INSTALL_DIR. It comes from the DAEMON
# repository's release, alongside the daemon binary it is built against. On
# macOS the addon is installed for consistency but Apple's system SQLite blocks
# extension loading, so the runtime reports the vector index unavailable and
# memory search stays literal — see docs/getting-started.md.
#
# The web UI is not a fourth binary and not a fourth service: it is a bundle of
# static files unpacked to $INSTALL_DIR/webui/<version>, which the daemon then
# serves on its own listener. The installer configures that by running
# `goodvibes-daemon webui enable` on the binary it just placed, so the settings
# keys live in one implementation rather than being copied into shell. The
# daemon's shipped binding is loopback, and `webui enable` does not change it,
# so a fresh install serves the web UI to THIS MACHINE ONLY. Reaching it from
# another device on your network is a deliberate separate act, printed in the
# receipt: `goodvibes-daemon webui enable --lan`.
#
# The whole install is pure-binary and checksum-verified; nothing is fetched
# through a package manager.
#
# Options (environment variables, so the pipe-to-sh form stays one command):
#   GOODVIBES_VERSION         install a specific terminal-app tag, e.g. v1.14.0 (default: latest)
#   GOODVIBES_DAEMON_VERSION  install a specific daemon tag (default: latest)
...
#   GOODVIBES_WEBUI           set to 0 to skip installing the browser operator surface (default: 1)
#   GOODVIBES_WEBUI_VERSION   install a specific web UI tag (default: latest)
...
# the browser driver dir, the unpacked web UI bundles,
...
#
# This file is versioned in the goodvibes-daemon repository at
# scripts/install.sh and published to goodvibes.sh by that repository's release
# workflow — it lives with the daemon because the daemon is the product every
# other one is installed alongside, and because the release lane that publishes
# it is that repository's. It is the ONLY copy: a second one in a consumer repo
# is exactly how two installers drift apart. The `/update` command re-runs the
# same download-verify-swap logic; keep the two in lockstep.

set -eu

DAEMON_REPO="mgd34msu/goodvibes-daemon"
REPO="mgd34msu/goodvibes-tui"
AGENT_REPO="mgd34msu/goodvibes-agent"
WEBUI_REPO="mgd34msu/goodvibes-webui"
INSTALL_DIR="${GOODVIBES_INSTALL_DIR:-$HOME/.local/bin}"
REQUESTED_VERSION="${GOODVIBES_VERSION:-latest}"
REQUESTED_DAEMON_VERSION="${GOODVIBES_DAEMON_VERSION:-latest}"
REQUESTED_AGENT_VERSION="${GOODVIBES_AGENT_VERSION:-latest}"
REQUESTED_WEBUI_VERSION="${GOODVIBES_WEBUI_VERSION:-latest}"
WITH_AGENT="${GOODVIBES_AGENT:-1}"
WITH_WEBUI="${GOODVIBES_WEBUI:-1}"
...
# Where the unpacked web UI bundles live: one versioned directory per installed
# version under the install tree, beside the binaries that serve them. Versioned
# because the daemon is configured to serve one directory by absolute path, and
# unpacking a new version over a running daemon's directory would swap files
# under it mid-request. A new version lands in its own directory, the daemon is
# pointed at it, and the previous one is removed only after that succeeds.
WEBUI_ROOT="$INSTALL_DIR/webui"

# Set by install_webui() once the daemon has actually been configured to serve
# the bundle, so the closing lines can name the web UI without claiming an
# install that did not finish. WEBUI_URL is lifted from the daemon's own
# receipt, never rebuilt here.
WEBUI_INSTALLED=0
WEBUI_URL=""
...
# --- the daemon: compiled binary from its own repository's release ---
# The daemon is its own product with its own repository and its own release
# line, so its tag is resolved separately and its files are verified against ITS
# manifest. Everything else about the placement is unchanged: an atomic rename
# over the target so a running binary is replaced cleanly, and the same
# $INSTALL_DIR every other product shares.
#
# This runs BEFORE the sqlite-vec addon (which comes from the same manifest) and
# before the wake-model provision (which runs this binary), and its manifest is
# left in $WORKDIR for both.
install_daemon() {
  say ""
  DAEMON_VERSION=$(resolve_tag "$DAEMON_REPO" "$REQUESTED_DAEMON_VERSION")
  DAEMON_BASE_URL="https://github.com/$DAEMON_REPO/releases/download/$DAEMON_VERSION"
  artifact="goodvibes-daemon-$PLATFORM_SUFFIX"

  say "Installing goodvibes-daemon $DAEMON_VERSION ..."
  fetch "$DAEMON_BASE_URL/SHA256SUMS.txt" "$WORKDIR/daemon-SHA256SUMS.txt"
  ...
  mv -f "$WORKDIR/$artifact" "$INSTALL_DIR/goodvibes-daemon"
  say "  installed  $INSTALL_DIR/goodvibes-daemon"

  if daemon_version_out=$("$INSTALL_DIR/goodvibes-daemon" --version 2>/dev/null); then
    say "  running:   $daemon_version_out"
  else
    fail "the installed goodvibes-daemon binary failed to run ('goodvibes-daemon --version')"
  fi
}
...
# goodvibes-daemon, and goodvibes-agent (they share $INSTALL_DIR). It is verified
# against the DAEMON repository's SHA256SUMS.txt, alongside the daemon binary it
# is built and released with — a missing manifest entry is a hard failure, never
# a skip. Placed with an atomic rename so a running process never dlopen()s a
# half-written file.
install_sqlite_vec() {
  [ "$WITH_VECTOR" = "1" ] || return 0
  addon_asset="sqlite-vec-${VEC_OS}-${arch_tag}.${VEC_SUFFIX}"

  # The addon is verified against the SAME SHA256SUMS.txt as the daemon binary,
  # so the manifest entry decides whether that release even ships it. A release
  ...
  fetch "$DAEMON_BASE_URL/$addon_asset" "$WORKDIR/$addon_asset"
...
# --- the browser operator surface: a bundle the daemon serves ---
#
# The web UI is static files, so it is not a fourth binary and it does not get a
# fourth service unit. The daemon already serves a bundle directory on its own
# listener, same origin as its API — which is exactly what makes the browser's
# same-origin policy a non-issue and means no CORS allowlist has to be widened
# for the app to talk to the daemon it was served by.
#
# The bundle is downloaded and verified against the web UI repository's own
# SHA256SUMS.txt on the same terms as everything else here: a missing manifest
# entry means that release predates the asset and is a note, not a failure; a
# present entry makes the download and its checksum mandatory. Extraction goes
# to a scratch directory and is moved into place only once complete, so an
# interrupted install never leaves a half-extracted directory that a daemon then
# serves as if it were an app.
#
# Each version unpacks into its OWN directory ($INSTALL_DIR/webui/<version>) and
# the daemon is pointed at it afterwards. Unpacking over the directory a running
# daemon is serving would swap files under it mid-request; this way the running
# daemon keeps serving the old directory until it is restarted onto the new one.
# Older versioned directories are pruned once the new one is configured.
#
# CONFIGURATION IS DONE BY THE DAEMON BINARY, not by writing settings from
# shell: `goodvibes-daemon webui enable --bundle-dir <dir>` owns the key names,
# the settings-tier routing and the "which listener actually serves this"
# question. Same reason `provision-wake-model` exists. It never widens network
# exposure — a daemon bound to loopback keeps serving to this machine only, and
# the receipt says so along with the one command that changes it.
install_webui() {
  [ "$WITH_WEBUI" = "1" ] || {
    say ""
    say "  note: skipping the web UI (GOODVIBES_WEBUI=0). Install it later with the"
    say "  same command, or serve a bundle you built yourself:"
    say "    goodvibes-daemon webui enable --bundle-dir <dir>"
    return 0
  }

  say ""
  WEBUI_VERSION=$(resolve_tag "$WEBUI_REPO" "$REQUESTED_WEBUI_VERSION")
  webui_base_url="https://github.com/$WEBUI_REPO/releases/download/$WEBUI_VERSION"
  # The tag carries a leading v; the asset name carries the bare version, the
  # same convention every packaged artifact in this platform uses.
  webui_plain_version="${WEBUI_VERSION#v}"
  webui_asset="goodvibes-webui-bundle-${webui_plain_version}.tar.gz"

  say "Installing the web UI $WEBUI_VERSION ..."
  fetch "$webui_base_url/SHA256SUMS.txt" "$WORKDIR/webui-SHA256SUMS.txt"
  ...
  say "  downloading $webui_asset ..."
  fetch "$webui_base_url/$webui_asset" "$WORKDIR/$webui_asset"

  actual=$(sha256_of "$WORKDIR/$webui_asset")
  [ "$expected" = "$actual" ] || fail "checksum mismatch for $webui_asset (expected $expected, got $actual)"
  say "  verified   $webui_asset"

  webui_staging="$WORKDIR/webui-staging"
  rm -rf "$webui_staging"
  mkdir -p "$webui_staging"
  tar -xzf "$WORKDIR/$webui_asset" -C "$webui_staging" ||
    fail "could not extract $webui_asset"
  ...
  mkdir -p "$WEBUI_ROOT"
  rm -rf "$webui_target.incoming"
  mv -f "$webui_staging/goodvibes-webui" "$webui_target.incoming"
  rm -rf "$webui_target"
  mv -f "$webui_target.incoming" "$webui_target"
  say "  installed  $webui_target"

  # Point the daemon at it. The binary owns the settings; this reads its receipt
  # and prints it, so the install's own output states the URL and the posture.
  # Not fatal: an unconfigured bundle on disk is a working install of everything
  # else plus one command to run, whereas an aborted install is neither.
  webui_output=$("$INSTALL_DIR/goodvibes-daemon" webui enable --bundle-dir "$webui_target" 2>&1) && webui_ok=1 || webui_ok=0
  if [ -n "$webui_output" ]; then
    printf '%s\n' "$webui_output" | while IFS= read -r webui_line; do
      say "  $webui_line"
    done
  fi
  if [ "$webui_ok" != "1" ]; then
    say "  NOTE: the bundle is on disk but the daemon was not configured to serve it. Run:"
    say "    goodvibes-daemon webui enable --bundle-dir $webui_target"
    return 0
  fi

  # The URL the daemon reported, lifted out of its own receipt rather than
  # rebuilt here from config: the command that wrote the settings is the one
  # that knows which listener serves the bundle and on what port, and a second
  # derivation in shell is how a printed URL starts disagreeing with the daemon.
  WEBUI_URL=$(printf '%s\n' "$webui_output" | awk '/^[[:space:]]*https?:\/\//{print $1; exit}')

  prune_old_webui_bundles "$webui_plain_version"
  WEBUI_INSTALLED=1
}

# Remove versioned bundle directories other than the one just configured. Runs
# only AFTER the daemon has been pointed at the new directory, so a failed
# configure never leaves the machine with no bundle at all.
prune_old_webui_bundles() {
  _keep="$1"
  [ -d "$WEBUI_ROOT" ] || return 0
  for _old in "$WEBUI_ROOT"/*; do
    [ -d "$_old" ] || continue
    [ "$(basename "$_old")" = "$_keep" ] && continue
    rm -rf "$_old"
    say "  removed    $_old (superseded)"
  done
}
...
run_uninstall() {
  ...
  # The unpacked web UI bundles. These are installer-placed program files, not
  # user data — the settings that point the daemon at them live under
  # ~/.goodvibes and are preserved with everything else there, so a re-install
  # finds its configuration intact and only has to put the files back.
  if [ -d "$WEBUI_ROOT" ]; then
    rm -rf "$WEBUI_ROOT"
    say "  removed    $WEBUI_ROOT"
    record_removed "$WEBUI_ROOT"
  fi
...
main() {
  ...
  say "Installing GoodVibes for $PLATFORM_SUFFIX into $INSTALL_DIR"
  say ""
  say "Installing goodvibes $VERSION ..."

  fetch "$BASE_URL/SHA256SUMS.txt" "$WORKDIR/SHA256SUMS.txt"

  artifact="goodvibes-$PLATFORM_SUFFIX"
  say "  downloading $artifact ..."
  ...
  chmod +x "$WORKDIR/$artifact"
  mv -f "$WORKDIR/$artifact" "$INSTALL_DIR/goodvibes"
  say "  installed  $INSTALL_DIR/goodvibes"

  # The daemon, from its own repository's release. It comes before the addon
  # (which rides its manifest), before the wake model (which runs this binary)
  # and before the web UI (which this binary configures).
  install_daemon
  ...
  # And again the same reason: unpack the web UI bundle and write the settings
  # that point the daemon at it BEFORE the restart below, so the daemon that
  # comes up is already serving it and the URL in the receipt works immediately
  ...
  if [ "$WEBUI_INSTALLED" = "1" ] && [ -n "$WEBUI_URL" ]; then
    # The posture line follows the URL the daemon actually reported, so a host

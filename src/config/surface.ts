 ...
 *
 * Because that is where the running daemon's state already is. The daemon was
 * built and shipped out of the terminal app's repository, so every store it has
 * written on every installed machine — sessions, approvals, watchers, devices,
 * channel policies, the code index — sits under `.goodvibes/tui/`. Renaming the
 * segment here would not move that state; it would make the daemon stop finding
 * it, silently, on machines that have been running for months.
 *
 ...
 * scoped move. That belongs to the distribution and migration stage of the
 * split, with the live-install rehearsal that goes with it. Until then this
 * constant carries the reason so nobody "tidies" it.
 */
